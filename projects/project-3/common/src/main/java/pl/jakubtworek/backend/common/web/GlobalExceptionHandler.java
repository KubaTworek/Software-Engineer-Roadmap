...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> validation(MethodArgumentNotValidException exception, HttpServletRequest request) {
        ...
        return error(HttpStatus.BAD_REQUEST, message, request, exception);
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    ResponseEntity<ApiError> malformedJson(HttpMessageNotReadableException exception, HttpServletRequest request) {
        return error(HttpStatus.BAD_REQUEST, "Malformed JSON request body", request, exception);
    }
    ...
    ResponseEntity<ApiError> unsupportedMediaType(HttpMediaTypeNotSupportedException exception, HttpServletRequest request) {
        return error(HttpStatus.UNSUPPORTED_MEDIA_TYPE, exception.getMessage(), request, exception);
    }
    ...
    ResponseEntity<ApiError> serviceUnavailable(IllegalStateException exception, HttpServletRequest request) {
        return error(HttpStatus.SERVICE_UNAVAILABLE, exception.getMessage(), request, exception);
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ApiError> generic(Exception exception, HttpServletRequest request) {
        return error(HttpStatus.INTERNAL_SERVER_ERROR, "Unexpected error", request, exception);
    }

    private ResponseEntity<ApiError> error(HttpStatus status, String message, HttpServletRequest request, Exception exception) {
        log.warn("request_failed status={} path={} message={} exception={}", status.value(), request.getRequestURI(), message, exception.toString(), exception);
