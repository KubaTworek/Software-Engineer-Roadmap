...
## Poprawka stabilności testów concurrency

W testach równoległych pula wątków musi mieć co najmniej tyle wątków, ile zadań czeka na wspólny start przez `CountDownLatch`.
Jeżeli uruchomimy 100 zadań na puli 24 wątków i każde z pierwszych 24 zadań zatrzyma się na latchu, pozostałe 76 zadań nigdy nie dojdzie do `ready.countDown()`. Test nie sprawdza wtedy race condition, tylko blokuje się na własnej barierze startowej.

Dlatego w `ConcurrencyStage2IntegrationTest` używamy `THREADS = REQUESTS`, aby wszystkie requesty mogły dojść do bariery, a dopiero potem ruszyć jednocześnie.
