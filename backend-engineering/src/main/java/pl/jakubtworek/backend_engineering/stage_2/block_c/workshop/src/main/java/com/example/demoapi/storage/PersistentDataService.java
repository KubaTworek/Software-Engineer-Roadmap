...
public class PersistentDataService {
    ...
    public Path write(String name, String content) throws IOException {
        Path directory = Path.of(properties.getPersistentDirectory()).toAbsolutePath().normalize();
    ...
    public String read(String name) throws IOException {
        Path file = resolveInside(
                Path.of(properties.getPersistentDirectory()).toAbsolutePath().normalize(),
                name
        );
    ...
    private static Path resolveInside(Path directory, String name) {
        ...
        if (!file.startsWith(directory)) {
            throw new IllegalArgumentException("file must stay inside persistent directory");
        }
        return file;
    }
}
